...
float imageColors[width][height][3];
int nthreads; 
scene_t scene; 
...
main( int argc, char **argv )
{
    ...
    pthread_t threads_here[nthreads];
    int *tid = malloc(sizeof(int)*nthreads); //allocating memory
    if (tid == NULL)
    {
    ...
    for (i = 0; i < nthreads; i++)//giving thread id values
    {
        tid[i] = i; 
    }

    for (i = 0; i < nthreads; i++)//creating threads
    {
        create_thread = pthread_create(&threads_here[i], NULL, threadFunction, (void*)(tid + i)); 
        if (create_thread != 0)
        {
    ...
    int k; 
    for (k = 0; k < nthreads; k++)//joining threads 
    {
        pthread_join(threads_here[k], NULL); 
    }

    int x, y; //printing out values
    for (x = 0; x < width; x++)
    {
        for (y = 0; y < height; y++)
        {
            printf( "%.0f %.0f %.0f\n", imageColors[x][y][0],
            imageColors[x][y][1], imageColors[x][y][2]);
        }
        printf("\n"); 
    }
    free_scene( &scene );
    ...
    free(tid); //free allocated memory

    return 0;
}

// typedef struct threadPixels{
//     int pixelpx; 
//     int pixelpy; 
// }threadPixels; 
void *threadFunction(void* arg)
{
    //threading here
    // threadPixels *threadData = (threadPixels*)arg; 
    // int px_s = threadData->pixelpx; 
    // int py_s = threadData->pixelpy; 
    //casts void arg to integer pointer and dereferences it
    int threadpx = *(int*) arg; 
    ...
    /* for every pixel */
                      //starts at the number of thread passed in
                    //and adds by the number of threades each iteration
    int px_s, py_s; 
    for( px_s=threadpx; px_s<width; px_s+=nthreads )
    {
        const double x = pixel_dx * ((double)( px_s-(width/2) ));
        for( py_s=0; py_s<height; ++py_s )
        {
            const double y = pixel_dy * ((double)( py_s-(height/2) ));
            Vec3 pixel_color;
            ...
            //store to use in main function
            imageColors[px_s][py_s][0] = scaled_color[0]; 
            imageColors[px_s][py_s][1] = scaled_color[1];
            imageColors[px_s][py_s][2] = scaled_color[2]; 
